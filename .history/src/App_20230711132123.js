...
function App() {
    ...
    function removeProduct(productName, e) {
        e.preventDefault();
        let newProducts = products.map(product => (product.name === productName && product.amount !== 0) ? product.amount-- : product)
        setProducts(newProducts);
    }

    function addProduct(productName, e) {
        e.preventDefault();
        let newProducts = products.map(product => product.name === productName ? [product, product.amount++] : product);
        setProducts(newProducts);
    }
