...
function App() {
    const [products, setProducts] = useState([{ name: 'Sashimi', amount: 0, url: 'https://pngimg.com/d/sushi_PNG9219.png'}, 
    ...
    function removeProduct(productName, e) {
        e.preventDefault();
        let newProducts = products.forEach(product => (product.name === productName && product.amount !== 0) ? product.amount-- : product)
        setProducts(newProducts);
    }

    function addProduct(productName, e) {
        e.preventDefault();
        let newProducts = products.forEach(product => product.name === productName ? product.amount++ : product);
        setProducts(newProducts);
    }
    ...
    return (
        <RouteSwitch products={products} removeProduct={removeProduct} addProduct={addProduct}/>
    );
}
